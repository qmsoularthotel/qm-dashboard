...
I link testuali ("stampa"/"sposta a incasso"/"correggi") impilati in una colonna stretta si accatastavano su più righe, illeggibili. Sostituiti con pulsanti circolari `.act-btn` (28px, sfondo `var(--navy-bg)`, icona stroke `var(--navy)`, tooltip `title`) affiancati in `.act-row` — stesse icone SVG di `ICON_STAMPA`/`ICON_SPOSTA`/`ICON_CORREGGI` in `reception.html`. Lato Compass (`app.js`), stesso pattern ma con i colori del design system principale (`var(--accent)`/`var(--accent-bg)`, non `--navy`/`--navy-bg` che lì non esistono) tramite l'helper `_receptionActBtn(icon,title,onclick)` e le costanti `RECEPTION_ICON_*`.
