...
**Bollicine nel liquido**: generate in `app.js` (array `cmBubbles`, 16 cerchi con posizione/durata/ritardo randomizzati ad ogni render) e clippate all'**intersezione** tra la sagoma della bottiglia (`cmBottleClip`) e il rettangolo del riempimento attuale (`cmLiquidClip`, stessi `fillY`/`fillH` del liquido) — così restano sempre dentro la zona di liquido vera, anche quando il livello sale/scende col progredire del giro (non clippate solo alla sagoma intera, altrimenti apparirebbero anche sopra il livello del liquido). Ogni bolla risale fino alla superficie del liquido con una distanza **dinamica** (`--cm-brise = cy - fillY`, variabile CSS per bolla), non un tragitto fisso — con la bottiglia piena (107 unità di liquido) un tragitto fisso corto (es. 26px) era quasi impercettibile, ora arriva fino a ~97 unità per le bolle che nascono vicino al fondo. Animazione `cmBubbleRise` in `style.css`, classe `.cm-bubble`, rispetta `prefers-reduced-motion`. Nessun bubble generato se `fillH<=4` (bottiglia praticamente vuota).
