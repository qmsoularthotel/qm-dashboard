...
- **Home — due numeri alla pari, non un totale grande**: scrivere solo il totale come cifra principale era fuorviante — un receptionist con 70€ fisici in cassa e 30€ di buoni in essere leggeva "€ 100,00" e lo interpretava come contante disponibile. Ora "Contanti in cassa" e "Buoni spesa in essere" sono due numeri dello stesso peso visivo (`.status-split`/`.split-val` in `reception.html`), con il totale relegato a una riga di testo più piccola sotto ("Totale fondo cassa: € X"). Stessa gerarchia lato Compass (`receptionRender()` in `app.js`).
