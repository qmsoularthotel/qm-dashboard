...
sez('Card "Punteggio medio": non deve contraddire Booking');
// 15/09/2026 — Art Resort: la card mostrava 8.5 (la stima del modello) mentre l'Extranet
// mostrava 8.6. Nessuno dei due numeri era sbagliato: le recensioni di inizio settembre
// erano gia' nel modello e non ancora nella cifra pubblicata da Booking. Ma la card
// presentava la stima come se fosse IL punteggio, quindi il QM leggeva due verita' in
// disaccordo senza sapere a chi credere. Ora il titolo e' la cifra LETTA su Booking, con
// la sua data, e la stima sta sotto dichiarata come tale.
(function () {
  var ORA = Date.now(), GG = 86400000;
  var box = {};
  function finto() {
    var o = { textContent: '', innerHTML: '',
      style: {}, classList: { contains: function () { return false; }, add: function () {}, remove: function () {} },
      closest: function () { return { title: '', style: {} }; } };
    return o;
  }
  var _getEl = document.getElementById;
  document.getElementById = function (id) {
    if (!/^(rev|revCatGrid|revPeriod)/.test(id)) return _getEl.call(document, id);
    if (!box[id]) box[id] = finto();
    return box[id];
  };
  var _calib = REV_CALIB, _hot = REV_HOTELS.sa, _sent = REV_SENT;
  REV_SENT = {};
  var rec = [];
  for (var i = 1; i <= 60; i++) rec.push({ _score: 9, _dateTs: ORA - i * 10 * GG, _date: new Date(ORA - i * 10 * GG), _hasReply: true });
  REV_HOTELS.sa = { data: rec, filtered: [], filter: 'all', sort: 'date_desc', search: '', page: 0 };

  function disegna(display, giorniFa) {
    box = {};
    REV_CALIB = { sa: { osservazioni: [{ ts: new Date(ORA - giorniFa * GG).toISOString(), display: display }],
      hl: 136, fascia: [100, 200], fonte: 'singolo', nUsate: 1 } };
    try { revRenderStats('sa'); } catch (e) { return { err: String(e) }; }
    return { titolo: box['rev-avg-sa'].textContent, sub: box['rev-avg-sub-sa'].textContent };
  }
  // Le recensioni sono tutte da 9: la stima mostra 9.0.
  var diverso = disegna(8.6, 1);
  ok('nessun errore nel disegno',          !diverso.err, true);
  ok('il titolo e\' la cifra di Booking',   diverso.titolo, '8.6');
  ok('e si vede da dove viene',            /su Booking/.test(diverso.sub), true);
  ok('con la data della lettura',          /letto il /.test(diverso.sub), true);
  ok('la stima resta visibile, come stima', /stima Compass 9\.0/.test(diverso.sub), true);
  ok('e si dice che Booking pubblica dopo', /ritardo/.test(diverso.sub), true);

  var uguale = disegna(9, 1);
  ok('quando coincidono lo dice',          /la stima coincide/.test(uguale.sub), true);
  ok('e il titolo resta quello',           uguale.titolo, '9.0');

  // Lettura vecchia: non puo' fare da titolo, sarebbe un numero fermo a settimane fa.
  var vecchia = disegna(8.6, REV_OSS_FRESCA_GG + 10);
  ok('lettura vecchia: titolo alla stima', vecchia.titolo, '9.0');
  ok('dichiarata stima',                   /^stima /.test(vecchia.sub), true);
  ok('e ricorda di registrare il punteggio', /non registrato da /.test(vecchia.sub), true);

  // Mai registrato nulla: comportamento di prima, con la parola "stima".
  box = {}; REV_CALIB = { sa: { osservazioni: [] } };
  var mai = {}; try { revRenderStats('sa'); mai = { titolo: box['rev-avg-sa'].textContent, sub: box['rev-avg-sub-sa'].textContent }; } catch (e) { mai = { err: String(e) }; }
  ok('senza osservazioni si mostra la stima', mai.titolo, '9.0');
  ok('e non si nomina Booking',               /su Booking/.test(mai.sub || ''), false);

  REV_CALIB = _calib; REV_HOTELS.sa = _hot; REV_SENT = _sent;
  document.getElementById = _getEl;
})();

sez('Da quando la stima ha smesso di mostrare quella cifra');
(function () {
  var ORA = Date.now(), GG = 86400000;
  var rec = [];
  for (var i = 1; i <= 60; i++) rec.push({ _score: 9, _dateTs: ORA - i * 10 * GG });
  for (var j = 0; j < 12; j++) rec.push({ _score: 1, _dateTs: ORA - j * GG / 4 });
  var oggi = Math.round(punteggioBooking(rec, 136, ORA).score * 10) / 10;
  var prima = Math.round(punteggioBooking(rec, 136, ORA - 5 * GG).score * 10) / 10;
  ok('le due cifre sono davvero diverse', oggi !== prima, true);
  var q = revStimaDaQuando(rec, 136, ORA, prima);
  ok('si sa da quando e\' cambiata',       q !== null && q <= ORA && q > ORA - 10 * GG, true);
  ok('se la cifra e\' quella di oggi, niente da raccontare', revStimaDaQuando(rec, 136, ORA, oggi), null);
  // Con una cifra ferma da sempre non c'e' NESSUN cambio da annunciare: senza l'uscita
  // anticipata si restituirebbe la data di oggi e la card direbbe "scesa il 15/9" su una
  // stima che non si e' mossa.
  var fermo = [];
  for (var k = 1; k <= 60; k++) fermo.push({ _score: 9, _dateTs: ORA - k * 10 * GG });
  ok('cifra ferma: nessun cambio annunciato', revStimaDaQuando(fermo, 136, ORA, 9), null);
  ok('una cifra mai mostrata non e\' un cambio recente', revStimaDaQuando(rec, 136, ORA, 2.2), null);
})();
