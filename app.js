  ...
  function renderHotel(label,data){
    ...
    let h=`<div style="margin-bottom:10px;">
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:6px;">
        ...
        ${tuttePartenze.length?`<span style="font-size:15px;font-weight:700;color:var(--amber);">🛫 ${tuttePartenze.length}</span>`:''}
...
async function revGenerateReply(p,gi){
  ...
  const prompt=`Sei Paolo P., Quality Manager del ${hotelName} a Napoli, un hotel 4 stelle con storia e carattere unico.\n\nDevi rispondere a questa recensione Booking.com in ${lang}.\n\nDATI RECENSIONE:\n- Ospite: ${guestName}\n- Punteggio: ${r._score}/10\n- Titolo: ${r['Titolo della recensione']||'—'}\n- Commento positivo: ${posText||'—'}\n- Commento negativo: ${negText||'—'}\n\nSTILE DI RIFERIMENTO — scrivi esattamente così:\n- Apertura originale, non formulaica: trova ogni volta un angolo diverso per iniziare (es. rifletti sul punteggio, su un dettaglio della recensione, sul legame con la struttura) — mai iniziare con "Thank you for your review" o formule simili\n- Usa la prima persona "I" quando prendi responsabilità diretta su un problema (es. "This is something I will address directly with the team")\n- Tono: Quality Manager di un 4 stelle — autorevole, caldo, diretto. Non gushing ("it really means a lot"), non burocratico ("meticulously", "seamless"), non da template\n- Critiche: riconosci con equilibrio ("you raise a fair point", "that is a valid observation") poi spiega il contesto e l'azione concreta\n- Chiusura evocativa: richiama lo spirito del posto, non solo "we look forward to welcoming you back"\n- Lunghezza: 3 paragrafi ben costruiti — non telegrafici, non eccessivi\n- Varia struttura e registro ad ogni risposta\n\nREGOLE TECNICHE:\n1. APERTURA: rivolgiti all'ospite per nome (es. "Dear ${guestName}," / "Cara/Caro ${guestName},")\n2. PUNTEGGIO: citalo solo se è alto (9-10/10) E la recensione è entusiasta, con un angolo originale — mai in modo banale\n3. PUNTI POSITIVI: richiama aspetti specifici citati, mai frasi generiche\n4. CRITICHE: spiegazione contestuale + azione concreta in prima persona\n5. CARATTERE STRUTTURA: quando pertinente, valorizza l'identità storica/unica\n6. VIETATO: non invitare mai al contatto diretto né alla prenotazione diretta (vietato dalle OTA)\n7. FIRMA su riga separata: "${firma}"\n8. Non ripetere le parole dell'ospite\n9. Rispondi SOLO con il testo, senza preamboli`;
  try{
    ...
    <div style="background:var(--surface);border-radius:12px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,.06);">
      ...
      <div style="padding:10px 14px;border-top:1px solid var(--border-light);display:flex;flex-direction:column;gap:8px;">
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
          ...
          <button onclick="navigator.clipboard.writeText(${JSON.stringify(waMsg)}).then(()=>{this.textContent='✓ Copiato!';setTimeout(()=>this.textContent='📋 Copia testo',2000);});" style="display:inline-flex;align-items:center;gap:6px;background:var(--surface);border:1px solid var(--border);border-radius:10px;padding:10px 16px;font-size:var(--fs-xs);color:var(--text);cursor:pointer;font-weight:600;">📋 Copia testo</button>
...
async function revExpGenerateReply(p,gi){
  ...
  const prompt=`Sei Paolo P., Quality Manager del ${hotelName} a Napoli, un hotel 4 stelle con storia e carattere unico.\n\nDevi rispondere a questa recensione ${platform} in ${lang}.\n\nDATI RECENSIONE:\n- Punteggio: ${r._score}/10\n- Titolo: ${r['review_title']||'—'}\n- Testo: ${r['review_text']||'(nessun testo)'}\n\nSTILE DI RIFERIMENTO — scrivi esattamente così:\n- Apertura originale, non formulaica: trova ogni volta un angolo diverso (es. rifletti sul punteggio, su un dettaglio della recensione, sul carattere della struttura) — mai iniziare con "Thank you for your review" o formule simili\n- Usa la prima persona "I" quando prendi responsabilità diretta su un problema (es. "This is something I will address directly with the team")\n- Tono: Quality Manager di un 4 stelle — autorevole, caldo, diretto. Non gushing, non burocratico, non da template\n- Critiche: riconosci con equilibrio ("you raise a fair point") poi spiega il contesto e l'azione concreta\n- Chiusura evocativa: richiama lo spirito del posto e dell'esperienza, non solo "we look forward to welcoming you back"\n- Varia struttura e registro ad ogni risposta\n\nREGOLE TECNICHE:\n1. APERTURA FISSA: inizia SEMPRE con "${italian?'Gentile ospite,':'Dear Guest,'}" — mai il nome (policy Expedia/Hotels.com)\n2. PUNTEGGIO: citalo solo se è alto (9-10/10) E la recensione è entusiasta, con angolo originale\n3. ${hasText?'LUNGHEZZA: 3 paragrafi ben costruiti — non telegrafici':'LUNGHEZZA: 2 frasi, varia il phrasing ad ogni risposta'}\n4. PUNTI POSITIVI: richiama aspetti specifici citati (colazione, posizione, staff, Galleria Umberto I, ascensore storico)\n5. CRITICHE: spiegazione contestuale + azione concreta in prima persona\n6. CARATTERE STRUTTURA: valorizza Galleria Umberto I, palazzo storico napoletano, ascensore d'epoca quando pertinente\n7. VIETATO: non invitare mai al contatto diretto né alla prenotazione diretta (vietato dalle OTA)\n8. FIRMA su riga separata: "${firma}"\n9. Non ripetere le parole dell'ospite\n10. Rispondi SOLO con il testo, senza preamboli`;
  try{
