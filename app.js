...
function renderPianoGiorno(elId,refDate,forceIdx){
  ...
  function renderHotel(label,data){
    ...
    if(partenze.length)h+=`<div style="margin-bottom:8px;">${groupLbl('Partenza senza arrivo')}<div style="display:flex;flex-wrap:wrap;gap:5px;">${partenze.map(r=>`<span style="background:var(--surface2);border:1px solid var(--border);color:var(--text);font-size:11px;font-weight:700;padding:4px 10px;border-radius:7px;">${r}</span>`).join('')}</div></div>`;
    if(cambi.length)h+=`<div style="margin-bottom:8px;">${groupLbl('Partenza con arrivo')}<div style="display:flex;flex-wrap:wrap;gap:5px;">${cambi.map(r=>`<span style="position:relative;background:var(--surface2);border:1px solid var(--border);color:var(--text);font-size:11px;font-weight:700;padding:4px 16px 4px 10px;border-radius:7px;">${r} ⇄<span style="position:absolute;top:3px;right:5px;width:6px;height:6px;border-radius:50%;background:var(--red);animation:ucDotBlink 1.1s ease-in-out infinite;"></span></span>`).join('')}</div></div>`;
...
function cmRender(state,key){
  ...
  const cmBottlePath='M25 11h14v24c7 5 11 11 13 17v72c0 4-3 7-7 7H19c-4 0-7-3-7-7V52c2-7 6-13 13-17V11z';
  const fillH=Math.round(117*pct);
  const fillY=14+(117-fillH);
  h+=`<div style="display:grid;grid-template-columns:130px 1fr;gap:26px;align-items:center;margin-bottom:16px;">
    <div style="position:relative;width:104px;height:247px;margin:0 auto;">
      <svg width="104" height="247" viewBox="0 0 64 152">
        <defs>
          ...
          <clipPath id="cmLogoBelow"><rect x="0" y="${fillY}" width="64" height="${152-fillY}"/></clipPath>
        </defs>
